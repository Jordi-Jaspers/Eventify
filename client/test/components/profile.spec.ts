...
test.describe('Profile Page Screenshots', () => {
    ...
    test('editable field focused', async ({ page }, testInfo) => {
        // Click the edit button for first name to enter edit mode
        const editButton = page.locator('button').filter({ hasText: '' }).first();
        
        // Find the pencil icon button for First Name field and click it
        const firstNameSection = page.locator('text=First Name').first();
        const editIcon = firstNameSection.locator('..').locator('button').first();
        ...
        const screenshotPath: string = getScreenshotPath('02-edit-mode', testInfo.project.name);
