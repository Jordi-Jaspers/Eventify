<script lang="ts">
	...
	let filters: MonitorFilters = $state(createDefaultFilters());
	...
	async function loadMonitorData(): Promise<void> {
		...
		try {
			...
			saveMonitorSession({ watchlistId, ...filters });
