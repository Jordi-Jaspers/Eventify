<script lang="ts">
    ...
    const label: string = $derived(
        getProviderMeta(provider.toUpperCase() as 'GOOGLE' | 'GITHUB').label
    );

    // Brand-colored SVG icons (intentionally different from the monochrome
    // lucide icons used elsewhere - these are official provider brand marks).
    const brandIcon: string = $derived({
