...
<Root bind:open={isDropdownOpen}>
	<Trigger>
		{#snippet child({ props })}
			<Button {...props} variant="ghost" size="icon" class="relative size-8 p-0" disabled={isUserSelf}>
...
<LockUserAction {id} {enabled} bind:isDropdownOpen bind:isLockDialogOpen />

<UpdateAuthorityAction {id} {authority} bind:isDropdownOpen bind:isUpdateDialogOpen />
