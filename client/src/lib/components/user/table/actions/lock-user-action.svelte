<script lang="ts">
	...
	import { users } from '$lib/store/global.js';
...
<Root bind:open={isLockDialogOpen} onOpenChange={() => (isDropdownOpen = false)}>
	<Content>
		...
		<Footer>
			...
			{#if enabled}
				<form method="POST" id="lock-user" action="?/lockUser" use:enhance={lockUser}>
					<Action type="submit" id="lock-user" class="bg-destructive text-destructive-foreground hover:bg-destructive/90">
						<input type="hidden" name="id" value={id} />
						{enabled ? 'Lock User' : 'Unlock User'}
					</Action>
				</form>
			{:else}
				<form method="POST" id="unlock-user" action="?/unlockUser" use:enhance={lockUser}>
					<Action type="submit" id="unlock-user" class="bg-primary text-primary-foreground hover:bg-primary/90">
						<input type="hidden" name="id" value={id} />
						{enabled ? 'Lock User' : 'Unlock User'}
					</Action>
				</form>
