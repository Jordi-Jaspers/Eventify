<script lang="ts">
	...
	interface SidebarMenuProps extends ComponentProps<typeof Root> {
		sidebarWidth?: number;
	}

	let { sidebarWidth = $bindable(0), ref = $bindable(null), collapsible = 'icon', ...restProps }: SidebarMenuProps = $props();
	$effect(() => {
		if (!ref) return;
